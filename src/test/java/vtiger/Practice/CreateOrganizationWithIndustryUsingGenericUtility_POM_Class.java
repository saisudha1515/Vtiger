...
public class CreateOrganizationWithIndustryUsingGenericUtility_POM_Class {
	public static void main(String[] args) throws Throwable {
		// Create objects of all utility classes (at the beginning it self when ever you
		// are using Generic Utilities make a habit of it)
		...
		String ORGNAME = eutil.readDataFromExcelFile("Organizations", 4, 2) + jUtil.getRandomNumber(); // instead of
																										// storing in a
																										// variable i am
																										// directly
																										// apending the
		...
		// step2: Launch the browser // It is exapmle for "RunTimePolymorphism" --
		// driver
		...
		/**
		 * POM CLASS AND BUSINESS LIBRARY IMPLEMENTATION here by using getters, the code
		 * is not getting optimized so in order to optimize the code we write business
		 * logic if necessary and if it possible to write business logic for all
		 * WebElements of Current web page.. upto now we encapsulated only webElements
		 * and Locators but we are not encapsulating the actions that are performing on
		 * those web elements..
		 */

		LoginPage lp = new LoginPage(driver); // at the time of object creation the caller has to provide the updated
												// driver..
		/* Performing login operation Using Business Library */
		// lp.loginToApplication(USERNAME, PASSWORD);

		/* performing login operation Using POM class in the script */
